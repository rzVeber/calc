...
public class Main {
    public static void main(String[] args) {
        ...
        do {
            ...
            switch (operate) {
                case "+" -> {
                    out = Funcs.sum(var1, var2);
                }
                case "-" -> {
                    out = Funcs.sub(var1, var2);
                }
                case "*" -> {
                    out = Funcs.mul(var1, var2);
                }
                case "/" -> {
                    out = Funcs.div(var1, var2);
                }
                default -> {
            ...
            System.out.println("Результат >> " + out + "\nEnter если хотите продолжить расчеты");
            isContinue = sc.nextLine().trim().equals("");
        } while (isContinue);
